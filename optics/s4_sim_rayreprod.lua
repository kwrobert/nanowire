...
function Simulator:build_device()
    ...
    local core_rad = self:getfloat('Parameters','nw_radius')
    print('Radius = '..core_rad)
    print('Period = '..vec_mag)
    -- Hexagonal nanowire
    local dx = core_rad*math.cos(math.pi/3)
    local dy = core_rad*math.sin(math.pi/3)
    local cent = vec_mag/2
    local verts = {cent+core_rad,cent,cent+dx,cent+dy,cent-dx,cent+dy,cent-core_rad,cent,cent-dx,cent-dy,cent+dx,cent-dy}
    -- The vertices are specified relative to the center of the polygon
    --local verts = {core_rad,0,dx,dy,-dx,dy,-core_rad,0,-dx,-dy,dx,-dy}
    self.sim:SetLayerPatternPolygon('nanowire_alshell','GaAs',{cent,cent},90,verts)
    --self.sim:SetLayerPatternCircle('nanowire_alshell','GaAs',{0,0},core_rad)
    --self.sim:SetLayerPatternCircle('nanowire_alshell','GaAs',{cent,cent},core_rad)
...
the optical properties of a single nanowire in a square lattice
    ...
    --pcall(load(S4.arg))
    --print(config_file)
    --if not pl.path.isfile(config_file) then
    --    error('The config file specified does not exist or is not a file')
    --end
