...
def get_incident_amplitude(freq,period,path):
    ...
    #plt.show()
    # Use the Poynting vector to go from power per unit area to E field mag per unit length
...
def postprocess_full3d(path,period,samples_xy):
    # Only works on a square grid for now
    dxy = period/samples_xy
    data = np.loadtxt(path+".E")
    ...
    #data = np.hstack((data,E_mag.reshape(E_mag.shape[0],1)))
    #E_mag_real = E_mag.real
    xs = data[:,0] 
    ys = data[:,1]
    zs = data[:,2]
    scatter3d(xs,ys,zs,E_mag)
    #fig = plt.figure(figsize=(8,6)) 
...
def postprocess_planes(path,period,samples_xy,plane):
    dxy = period/samples_xy
    data = np.loadtxt(path+".E")
    ...
    #data = np.hstack((data,E_mag.reshape(E_mag.shape[0],1)))
    #E_mag_real = E_mag.real
    mat = np.column_stack((data[:,0],data[:,1],data[:,2],E_mag))
    planes = np.array([row for row in mat if row[0] == plane or row[1] == plane])
    planes[:,0] = planes[:,0]*dxy
    planes[:,1] = planes[:,1]*dxy
...
def build_sim(conf):
    ...
    num_basis = conf.getint("Simulation","numbasis") 
    ...
    # LAYER SPECIFIED
    sim.AddLayer(Name='air',Thickness=conf.getfloat('Geometry','air_t'),Material='vacuum')
    sim.AddLayer(Name='ito',Thickness=conf.getfloat('Geometry','ito_t'),Material='ito')
    sim.AddLayer(Name='nanowire',Thickness=conf.getfloat('Geometry','nw_height'),Material='cyclotrene')
    sim.AddLayer(Name='substrate',Thickness=conf.getfloat('Geometry','substrate_t'),Material='gaas')
    
    # Add patterning to layers
    core_rad = conf.getfloat('Geometry','nw_radius')
    alinp_rad = core_rad + conf.getfloat('Geometry','shell_t')
    ...
    # Set frequency
    f_phys = conf.getfloat("Simulation","frequency")
    c_conv = constants.c/conf.getfloat("Simulation","base_unit")
    f_conv = f_phys/c_conv
    print(f_conv)
    sim.SetFrequency(f_conv)

    # Define incident light. Normally incident with frequency dependent amplitude
    E_mag = get_incident_amplitude(f_phys,vec_mag,conf.get("Simulation","input_power"))
    ...
    E_layers = []
    output_file = conf.get("Simulation","base_name")
    existing_files = glob.glob(output_file+".*")
    if existing_files:
        for afile in existing_files:
            os.remove(afile)
    for z in np.linspace(0,2.3,46):
        sim.GetFieldsOnGrid(z,NumSamples=(50,50),Format='FileAppend',BaseFilename=output_file)
